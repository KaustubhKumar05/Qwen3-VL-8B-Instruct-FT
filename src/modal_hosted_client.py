"""modal-hosted vLLM client for vision LLM interactions."""
...
class ModalHostedClient:
    """Client for interacting with modal-hosted vLLM endpoint on Modal."""
    ...
    def __init__(
        ...
        """
        Initialize the modal-hosted vLLM client.
    ...
    def warmup(self) -> bool:
        ...
        print(f"Warming up modal-hosted endpoint at {self.base_url}...")
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    print(f"modal-hosted endpoint ready after {attempt} attempt(s)")
        ...
        print("Failed to warm up modal-hosted endpoint")
        return False
    ...
    def analyze_image(
        ...
        """
        Analyze a kitchen image using the modal-hosted vision model.
        ...
        Returns:
            Dictionary containing:
                ...
                - cost: None (modal-hosted, no per-request cost)
        """
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                return {
                    ...
                    "usage": usage,
                    "cost": None,  # modal-hosted, no per-request cost
                    "model": f"modal-hosted/{self.model_name}",
                }
                ...
                raise Exception(f"Error calling modal-hosted vLLM endpoint: {str(e)}")

        raise Exception(
            f"modal-hosted endpoint failed after {self.max_retries} retries: {last_error}"
        )
